...
<div x-data="{
    ...
    <div class="print:hidden flex flex-wrap items-center justify-between gap-4 mb-6">
        <div class="flex items-center gap-3">
            <a href="{{ route('download.statement.customer', $customer->id) }}" class="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 text-primary-dark font-semibold rounded-lg hover:bg-gray-50 transition-all shadow-sm text-sm">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('customer.index') }}" class="flex items-center gap-2 px-5 py-2 bg-primary text-white font-semibold rounded-lg hover:bg-primary/90 transition-all shadow-sm text-sm">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6 md:p-8 print:p-0 print:border-none print:shadow-none print:rounded-none w-full max-w-full overflow-x-hidden">
        ...
        <!-- Statement Context Info & Customer Details -->
        <div class="flex flex-col md:flex-row justify-between mb-8 gap-6">
            <div>
                <h6 class="text-xs font-bold text-primary-dark uppercase tracking-wider mb-2">Customer Details</h6>
                <h2 class="text-xl font-bold text-primary-dark">{{ $customer->name }}</h2>
                <div class="text-xs font-semibold text-gray-400 mt-2 space-y-1">
                    <p>Customer ID: <span class="text-primary">{{ $customer->customer_code ?? 'CUS-' . date('Y') . '-' . str_pad($customer->id, 3, '0', STR_PAD_LEFT) }}</span></p>
                    <p>Phone: <span class="text-gray-900">{{ $customer->phone ?? '-' }}</span></p>
                </div>
            </div>
            <div class="md:text-right bg-gray-50 rounded-xl p-4 border border-gray-100 h-fit w-full sm:w-auto sm:min-w-[300px]">
                <div class="space-y-2">
                    ...
                    <div class="flex justify-between gap-4 sm:gap-8 md:justify-end">
                        ...
                        <span class="text-xs font-bold text-primary-dark">STMT-{{ date('Y') }}-{{ $customer->id }}</span>
                    </div>
                </div>
            </div>
        </div>
                ...
                <table class="w-full text-left whitespace-nowrap">
                    ...
                    <tbody class="divide-y divide-gray-50 text-xs font-medium text-gray-700">
                        @php
                            ...
                            foreach($customer->orders as $order) {
                                ...
                                if($order->paid_amount > 0) {
                                    $transactions->push([
                                        ...
                                        'download_route' => null // Could link to same invoice if needed
                                    ]);
                                }
                            }
                            ...
                            foreach($customer->payments as $payment) {
                                ...
                                $transactions->push([
                                    ...
                                    'download_route' => route('payment_in.download', $payment->id)
                                ]);
                            }
                        ...
                        <tr class="bg-gray-50/50">
                            <td class="px-4 py-3 font-semibold">{{ \Carbon\Carbon::parse($customer->created_at)->format('M j, Y') }}</td>
                            <td class="px-4 py-3">
                        ...
                        @forelse($sorted as $txn)
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors group">
                                ...
                                <td class="px-4 py-3">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold {{ $txn['type'] === 'Invoice' ? 'bg-primary/10 text-primary uppercase' : 'bg-accent/10 text-accent uppercase' }}">
                                        {{ $txn['type'] }}
                                    </span>
                                </td>
                                <td x-show="opts.description" class="px-4 py-3 whitespace-normal min-w-[200px]">
                                    <div class="flex items-center justify-between gap-4">
                                        ...
                                        <div class="print:hidden opacity-0 group-hover:opacity-100 transition-opacity">
                                            <a href="{{ $txn['download_route'] }}" class="text-[10px] font-bold text-primary hover:underline bg-primary/5 px-2 py-1 rounded">
                                                <i class="bi bi-file-earmark-pdf mr-0.5"></i> PDF
                                            </a>
                                        </div>
                                        @endif
                                    </div>
            ...
            <div class="md:w-[40%] p-6 rounded-xl border border-gray-100 bg-gray-50/50">
                ...
                <div class="space-y-3 text-sm font-semibold text-gray-500">
                    ...
                    <div class="flex justify-between items-center border-b border-gray-100/70 pb-2">
                        ...
                        <strong class="text-gray-800">{{ $customer->orders ? $customer->orders->count() : 0 }}</strong>
                    </div>
                    <div class="flex justify-between items-center border-b border-gray-100/70 pb-2">
                        <span>Total Payments:</span>
                        <strong class="text-gray-800">{{ $customer->payments ? $customer->payments->count() : 0 }}</strong>
                    </div>
                </div>
            </div>
