            ...
            <form action="{{ route('view_payment_out') }}" method="GET">
                <div
                    ...
                    <div class="relative group flex-1 min-w-[200px]">
                        <i
                            class="bi bi-search absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm group-focus-within:text-primary transition-colors"></i>
                        <input type="text" name="search" placeholder="Search vouchers or suppliers..."
                    ...
                    <div class="relative min-w-[150px]">
                        ...
                        <i
                            class="bi bi-chevron-down absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none text-[10px]"></i>
                    </div>
                    ...
                    <div class="relative min-w-[150px]">
                        <select name="method" onchange="this.form.submit()"
                            ...
                            <option value="Bank Transfer" {{ request('method') == 'Bank Transfer' ? 'selected' : '' }}>Bank
                            </option>
                ...
                <table class="w-full whitespace-nowrap text-left">
                    <thead class="sticky top-0 z-10">
                        <tr class="bg-white border-b border-gray-100">
                            <th
                                class="px-5 py-4 text-[12px] font-black text-primary-dark uppercase tracking-wider w-16 text-center">
                                #</th>
                            <th class="px-5 py-4 text-[12px] font-black text-primary-dark uppercase tracking-wider">Voucher
                                Info</th>
                            <th class="px-5 py-4 text-[12px] font-black text-primary-dark uppercase tracking-wider">Supplier
                            </th>
                            <th class="px-5 py-4 text-[12px] font-black text-primary-dark uppercase tracking-wider">Product
                                Name</th>
                            <th
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex flex-col gap-1 max-w-[220px]">
                                        @php
                                            // 1. Try SupplierPaymentDetail → bill → items (when payment was linked to a specific bill)
                                            $productNames = $item->details
                                                ->filter(fn($d) => $d->bill && $d->bill->items->count())
                                                ->flatMap(fn($d) => $d->bill->items->pluck('product_name'))
                                                ->unique();

                                            // 2. Fallback: all purchase bill items for this supplier
                                            if ($productNames->isEmpty()) {
                                                $productNames = ($billItemsBySupplier[$item->supplier_id] ?? collect())
                                                    ->flatMap(fn($bill) => $bill->items->pluck('product_name'))
                                                    ->unique();
                                            }
                                        @endphp
                                        @forelse($productNames as $pname)
                                            <span class="text-[12px] font-semibold text-primary-dark whitespace-normal">{{ $pname }}</span>
                                        @empty
                                            <span class="text-[11px] text-gray-400 font-semibold italic">—</span>
                                        @endforelse
                                    </div>
                                </td>
                                <td class="px-5 py-4 text-[12px] font-semibold text-primary-dark text-center">
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex items-center justify-end gap-1.5">
                                        ...
                                        <a href="{{ route('supplier.statement', $item->supplier_id) }}" class="btn-action-edit"
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="px-5 py-16 text-center text-gray-400">
                                    ...
                                    <p class="text-[13px] font-bold uppercase tracking-widest text-gray-400">No payment records
                                        found</p>
                                </td>
                            </tr>
                        @endforelse
                        ...
                        <div class="flex flex-col lg:flex-row gap-8 w-full">
                            ...
                            <div class="w-full lg:w-2/3 space-y-6">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 w-full">
                                    <div class="space-y-1.5 w-full">
                                        ...
                                        <div class="relative group w-full">
                                            ...
                                            <i
                                                class="bi bi-chevron-down absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none text-[10px]"></i>
                                        </div>
                                        <div
                                            class="mt-2 flex items-center justify-between bg-primary/10 px-3 py-1.5 rounded-md border border-primary/20">
                                            <span
                                                class="text-[10px] font-bold text-primary uppercase tracking-tighter">Current
                                                Payable:</span>
                                            <span class="text-[12px] font-black text-primary">{{ $symbol }} <span
                                                    x-text="supplierBalance">0.00</span></span>
                                        </div>
                                    </div>

                                    <div class="space-y-1.5 w-full">
                                        <label
                                            class="text-[11px] font-bold text-gray-700 uppercase tracking-wider">Reference /
                                            Bill #</label>
                                        <div class="relative group w-full">
                                            <input type="text" name="reference" placeholder="e.g. BILL-001"
                                                x-model="reference"
                                            ...
                                            <i
                                                class="bi bi-hash absolute right-4 top-1/2 -translate-y-1/2 text-gray-400"></i>
                                        </div>
                                        <p class="text-[10px] text-gray-400 font-medium italic mt-1">* Suggested PV:
                                            {{ $suggestedVoucherNo }}</p>
                                    </div>
                                </div>

                                <div class="grid grid-cols-1 md:grid-cols-3 gap-5 w-full">
                                    <div class="space-y-1.5 w-full">
                                        <label class="text-[11px] font-bold text-gray-700 uppercase tracking-wider">Bank /
                                            Cash Account <span class="text-primary">*</span></label>
                                        <div class="relative">
                                    ...
                                    <div class="space-y-1.5 w-full">
                                        <label class="text-[11px] font-bold text-gray-700 uppercase tracking-wider">Amount
                                            Paid <span class="text-primary">*</span></label>
                                        <div class="relative group w-full">
                                            <span
                                                class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-xs">{{ $symbol }}</span>
                                    ...
                                    <div class="space-y-1.5 w-full">
                                        <label class="text-[11px] font-bold text-gray-700 uppercase tracking-wider">Payment
                                            Date</label>
                                ...
                                <div class="space-y-1.5 pt-2 w-full">
                                    <label class="text-[11px] font-bold text-gray-700 uppercase tracking-wider">Internal
                                        Notes</label>
                            ...
                            <div class="w-full lg:w-1/3">
                                <div
                                    class="bg-gray-50 border border-gray-200 rounded-2xl p-6 h-full flex flex-col gap-6 lg:sticky lg:top-0">
                                    <div class="flex items-center justify-between border-b border-gray-200 pb-4">
                                        <h4 class="text-[11px] font-black text-primary-dark uppercase tracking-widest">Live
                                            Voucher Preview</h4>
                                    ...
                                    <div class="space-y-3">
                                        <div class="p-3 bg-white border border-gray-100 rounded-xl shadow-sm">
                                            <p class="text-[9px] font-bold text-gray-400 uppercase mb-1">Pay To (Supplier)
                                            </p>
                                            <p class="text-[12px] font-black text-primary-dark truncate"
                                                x-text="supplierName">SELECT SUPPLIER</p>
                                        </div>
                                        <div class="p-3 bg-white border border-gray-100 rounded-xl shadow-sm">
                                            <p class="text-[9px] font-bold text-gray-400 uppercase mb-1">Transaction Method
                                            </p>
                                            <p class="text-[11px] font-black text-gray-600 uppercase"
                                                x-text="paymentMethod">CASH PAYMENT</p>
                                        </div>
                                        <div
                                            class="p-3 bg-accent/10 border border-accent/20 rounded-xl shadow-sm flex items-center justify-between">
                                            <div>
                                                <p class="text-[9px] font-bold text-accent uppercase mb-1">System Status</p>
                                                <p
                                                    class="text-[10px] font-black text-accent uppercase tracking-widest flex items-center gap-1.5">
                                                    <span
                                                        class="w-1.5 h-1.5 bg-accent/10 rounded-full animate-pulse"></span>
                                                    Valid Voucher
                                                </p>
                                            </div>
                                    ...
                                    <div class="mt-auto pt-6 border-t border-gray-200">
                                        <div class="bg-accent rounded-xl p-5 shadow-lg relative overflow-hidden group">
                                            ...
                                            <p
                                                class="text-[10px] font-bold text-white uppercase tracking-widest mb-1.5 opacity-90">
                                                Amount to Debit</p>
                                            <div class="flex items-baseline gap-1 text-white">
                                                <span class="text-lg font-bold opacity-90">{{ $symbol }}</span>
                                                <h2 class="text-3xl font-black tracking-tight"
                                                    x-text="parseFloat(paymentAmount || 0).toLocaleString(undefined, {minimumFractionDigits: 2})">
                                                    0.00</h2>
                                            </div>
                                            <div
                                                class="mt-4 flex items-center justify-between text-[10px] text-white font-bold uppercase">
                                                <span class="opacity-80"
                                                    x-text="new Date(paymentDate).toLocaleDateString(undefined, { day: 'numeric', month: 'short', year: 'numeric' })"></span>
