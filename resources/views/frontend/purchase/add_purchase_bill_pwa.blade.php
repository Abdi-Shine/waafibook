...
<div class="pb-28 bg-background min-h-screen" x-data="{
    ...
    async submitNewSupplier() {
        ...
        try {
            ...
            this.suppliers.push({ id: s.id, name: s.name, phone: s.phone || this.newSupplierForm.phone || '', balance: parseFloat(s.amount_balance) || 0 });
            this.supplierId = s.id;
        ...
        <div class="grid grid-cols-2 gap-3 mb-1.5">
            <div>
                ...
                <button type="button" @click="showSupplierPicker = true; partySearch = ''"
                    class="w-full flex items-center justify-between px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-[14px] font-medium outline-none text-left"
                    :class="supplierId ? 'text-gray-700' : 'text-gray-400'">
                    <span class="truncate" x-text="supplierId ? (suppliers.find(s => s.id == supplierId)?.name || 'Select supplier') : 'Select supplier'"></span>
