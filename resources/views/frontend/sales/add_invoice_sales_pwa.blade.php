...
<div class="pb-28 bg-background min-h-screen" x-data="{
    ...
    newCustomerForm: { name: '', phone: '', amount_balance: '' },
    ...
    customers: @js($customers->map(fn ($c) => ['id' => $c->id, 'name' => $c->name, 'phone' => $c->phone, 'balance' => (float) ($c->amount_balance ?? 0)])),
    items: [{ product_id: '', product_name: '', product_code: '', unit: 'Piece', quantity: 1, unit_price: 0 }],
    showCustomerPicker: false,
    partySearch: '',

    get filteredCustomers() {
        if (!this.partySearch) return this.customers;
        const q = this.partySearch.toLowerCase();
        return this.customers.filter(c => c.name.toLowerCase().includes(q) || (c.phone || '').toLowerCase().includes(q));
    },
    selectCustomer(c) {
    ...
    get subtotal() {
        return this.items.reduce((sum, it) => sum + Math.max(0, (parseFloat(it.quantity) || 0) * (parseFloat(it.unit_price) || 0)), 0);
    },
    get grandTotal() {
    ...
    lineAmount(it) {
        return Math.max(0, (parseFloat(it.quantity) || 0) * (parseFloat(it.unit_price) || 0));
    },
    addItem() {
        this.items.push({ product_id: '', product_name: '', product_code: '', unit: 'Piece', quantity: 1, unit_price: 0 });
    },
    removeItem(i) {
    ...
    async submitNewProduct() {
        ...
        try {
            ...
            const mapped = { id: p.id, name: p.name, code: p.code, unit: p.unit || 'Piece', price: parseFloat(p.selling_price) || 0, stock: parseFloat(p.stock) || 0 };
            this.products.push(mapped);
            this.showAddProductModal = false;
            if (this.pickingItemIndex !== null) this.pickProduct(mapped);
    ...
    openAddCustomerModal() {
        this.newCustomerForm = { name: '', phone: '', amount_balance: '' };
        this.showAddCustomerModal = true;
    },
    async submitNewCustomer() {
        this.savingCustomer = true;
        try {
            const response = await fetch('{{ route('sales.quick.customer') }}', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
                ...
                body: JSON.stringify(this.newCustomerForm),
            });
            const data = await response.json();
            if (response.status === 422) {
                const firstError = Object.values(data.errors || {})[0]?.[0] || 'Please check the form.';
                Swal.fire({ icon: 'error', title: 'Could not add customer', text: firstError });
                return;
            }
            if (!response.ok) {
                Swal.fire({ icon: 'error', title: 'Something went wrong', text: data.message || 'Please try again.' });
            ...
            this.customers.push({ id: data.id, name: data.name, phone: data.phone || this.newCustomerForm.phone || '', balance: parseFloat(data.balance) || 0 });
            this.customerId = data.id;
            this.customerBalance = parseFloat(data.balance) || 0;
            this.showAddCustomerModal = false;
            Swal.fire({ toast: true, position: 'top', icon: 'success', title: data.name + ' added', timer: 1500, showConfirmButton: false });
        } catch (e) {
            Swal.fire({ icon: 'error', title: 'Network error', text: 'Could not add customer. Please try again.' });
        } finally {
            this.savingCustomer = false;
        }
    },
    async submitInvoice() {
        if (this.saleType === 'credit' && !this.customerId) {
            Swal.fire({ icon: 'error', title: 'Customer required', text: 'Please select a customer for credit sales.' });
            return;
        }
        ...
        try {
            ...
            if (this.saleType === 'cash' && (this.cashName || this.cashPhone)) {
                let details = [];
                if (this.cashName) details.push('Name: ' + this.cashName);
                if (this.cashPhone) details.push('Phone: ' + this.cashPhone);
                notes = 'WALK-IN DETAILS: \n' + details.join(' | ') + '\n----------------\n' + notes;
            }
            const payload = {
                customer_id: this.saleType === 'credit' ? this.customerId : null,
                ...
                items: validItems.map(it => ({
                    ...
                    discount: 0,
                    total_price: this.lineAmount(it),
                })),
            };
            const response = await fetch('{{ route('sales.invoice.store') }}', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
            ...
            Swal.fire({ icon: 'success', title: 'Invoice Created!', confirmButtonColor: '#004161' })
                .then(() => window.location.href = '{{ route('sales.invoice.view') }}');
        } catch (e) {
            Swal.fire({ icon: 'error', title: 'Network error', text: 'Could not save the invoice. Please try again.' });
        } finally {
            this.saving = false;
        }
    }
    ...
    <div class="flex items-center justify-between gap-3 px-5 pt-4">
        <h1 class="text-[16px] font-black text-primary-dark">Sale Invoice</h1>
    ...
    <div class="mx-5 mt-4 bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        <div class="flex items-center justify-between mb-3">
            <p class="text-[11px] font-black text-gray-400 uppercase tracking-wide">Customer & Sale Info</p>
            <div class="grid grid-cols-2 gap-1 bg-gray-100 rounded-lg p-1">
                <button type="button" @click="saleType = 'credit'"
                    :class="saleType === 'credit' ? 'bg-primary text-white' : 'text-gray-500'"
                    class="px-3 py-1 rounded-md text-[11px] font-bold uppercase tracking-wide transition-all">Credit</button>
                <button type="button" @click="saleType = 'cash'"
                    :class="saleType === 'cash' ? 'bg-accent text-primary' : 'text-gray-500'"
                    class="px-3 py-1 rounded-md text-[11px] font-bold uppercase tracking-wide transition-all">Cash</button>
            </div>
        </div>

        <template x-if="saleType === 'credit'">
            <div class="mb-3">
                <label class="text-[11px] font-bold text-gray-700 uppercase tracking-wider mb-1.5 block">Customer <span class="text-primary">*</span></label>
                <button type="button" @click="showCustomerPicker = true; partySearch = ''"
                    ...
                    :class="customerId ? 'text-gray-700' : 'text-gray-400'">
                    <span class="truncate" x-text="customerId ? (customers.find(c => c.id == customerId)?.name || 'Select customer') : 'Select customer'"></span>
                    <i class="bi bi-chevron-down text-gray-400 text-xs shrink-0 ml-2"></i>
                </button>
                <div class="flex items-center justify-between mt-1.5">
                    <p class="text-[11px] font-black text-accent uppercase tracking-tight" x-show="customerId">
                        BAL: <span x-text="'{{ $curr }} ' + customerBalance.toFixed(2)"></span>
                    </p>
                    <button type="button" @click="openAddCustomerModal()" class="text-[11px] font-bold text-primary ml-auto">
        ...
        <template x-if="saleType === 'cash'">
            <div class="grid grid-cols-2 gap-3 mb-3">
                <div>
                    <label class="text-[11px] font-bold text-gray-700 uppercase tracking-wider mb-1.5 block">Billing Name</label>
                    <input type="text" x-model="cashName" placeholder="Optional"
                        class="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-[14px] font-medium text-gray-700 outline-none">
                </div>
                <div>
                    <label class="text-[11px] font-bold text-gray-700 uppercase tracking-wider mb-1.5 block">Phone No.</label>
                    <input type="text" x-model="cashPhone" inputmode="numeric" oninput="this.value = this.value.replace(/[^0-9]/g, '')" placeholder="Optional"
                        class="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-[14px] font-medium text-gray-700 outline-none">
                </div>
            </div>
        </template>
        ...
        <div>
            <label class="text-[11px] font-bold text-gray-700 uppercase tracking-wider mb-1.5 block">Invoice Date <span class="text-primary">*</span></label>
            <input type="date" x-model="invoiceDate" required
    ...
    <div class="mx-5 mt-4 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-4 py-3 flex items-center gap-2 border-b border-gray-100 bg-background/50">
            ...
            <h2 class="text-[11px] font-black text-primary-dark uppercase tracking-wider">Sale Items</h2>
        </div>
            ...
            <table class="w-full text-left" style="min-width: 600px;">
                <thead>
                    <tr class="bg-white border-b border-gray-100">
                        ...
                        <th class="px-3 py-3 text-[10px] font-black text-primary-dark uppercase tracking-wider border-r border-gray-100" style="min-width: 200px;">Item</th>
    ...
    {{-- Summary --}}
    <div class="mx-5 mt-4 bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        <p class="text-[11px] font-black text-gray-400 uppercase tracking-wide mb-3">Invoice Summary</p>
    ...
    <div class="mx-5 mt-5 flex gap-3">
        ...
        <button type="button" @click="submitInvoice()" :disabled="saving"
            ...
            <i class="bi" :class="saving ? 'bi-arrow-repeat animate-spin' : 'bi-check2-all'"></i>
    ...
    <div x-show="pickingItemIndex !== null" x-cloak x-transition.opacity
        ...
        <div x-show="pickingItemIndex !== null" x-transition:enter="transition ease-out duration-250"
            ...
            <div class="flex items-center px-4 py-2 border-y border-gray-100 bg-background/50 shrink-0">
                ...
                <span class="w-16 text-[9px] font-black text-primary-dark uppercase tracking-wider text-right">Price</span>
                <span class="w-14 text-[9px] font-black text-primary-dark uppercase tracking-wider text-right">Stock</span>
            </div>
            ...
            <div class="overflow-y-auto flex-1">
                <template x-for="p in filteredProducts" :key="p.id">
                    <button type="button" @click="pickProduct(p)"
                        ...
                        <span class="w-16 text-[12px] font-bold text-primary-dark text-right" x-text="p.price.toFixed(0)"></span>
                        <span class="w-14 text-[12px] font-bold text-accent text-right" x-text="p.stock"></span>
                    </button>
                </template>
    ...
    {{-- Select Customer — mobile bottom sheet --}}
    <div x-show="showCustomerPicker" x-cloak x-transition.opacity
        class="fixed inset-0 z-[80] bg-slate-900/40" @click.self="showCustomerPicker = false">
        <div x-show="showCustomerPicker" x-transition:enter="transition ease-out duration-250"
            x-transition:enter-start="translate-y-full" x-transition:enter-end="translate-y-0"
            x-transition:leave="transition ease-in duration-200"
            x-transition:leave-start="translate-y-0" x-transition:leave-end="translate-y-full"
            class="absolute bottom-0 left-0 right-0 bg-white rounded-t-[1.5rem] max-h-[85vh] flex flex-col">
            ...
            <div class="px-5 py-4 bg-primary flex items-center justify-between shrink-0">
                <h2 class="text-white font-bold text-[16px]">Select Customer</h2>
                <button @click="showCustomerPicker = false" class="w-8 h-8 bg-white/10 rounded-lg text-white flex items-center justify-center">
                    <i class="bi bi-x-lg text-xs"></i>
                </button>
            </div>
            ...
            <div class="px-4 py-3 shrink-0">
                <div class="flex items-center gap-2 px-3 py-2.5 bg-gray-100 rounded-xl">
                    <i class="bi bi-search text-gray-400 text-sm"></i>
                    <input type="text" x-model="partySearch" placeholder="Search customer..."
                        class="flex-1 text-[13px] text-gray-700 font-medium tracking-wide placeholder-gray-400 outline-none border-none ring-0 bg-transparent"
                        autocomplete="off">
                </div>
            </div>
            ...
            <div class="overflow-y-auto flex-1">
                <template x-for="c in filteredCustomers" :key="c.id">
                    <button type="button" @click="selectCustomer(c)"
                        ...
                        <div class="min-w-0">
                            <p class="text-[13px] font-black text-primary-dark truncate" x-text="c.name"></p>
                            <p class="text-[11px] text-gray-400" x-show="c.phone" x-text="c.phone"></p>
                        </div>
                        <div class="flex items-center gap-1.5 shrink-0">
                            <span class="text-[12px] font-bold text-accent" x-text="c.balance.toLocaleString('en', {minimumFractionDigits: 0, maximumFractionDigits: 2})"></span>
                            <span class="w-5 h-5 rounded flex items-center justify-center text-white bg-accent">
                                <i class="bi bi-arrow-down-left text-[10px]"></i>
                            </span>
                        </div>
                    </button>
                </template>
                <template x-if="!filteredCustomers.length">
                    <div class="py-10 text-center">
                        <i class="bi bi-person-x text-3xl text-gray-300"></i>
                        <p class="text-sm text-text-secondary mt-2 font-semibold">No customers found.</p>
                    </div>
                </template>
            </div>
        </div>
    ...
    {{-- Add Customer — mobile bottom sheet --}}
    <div x-show="showAddCustomerModal" x-cloak x-transition.opacity
        class="fixed inset-0 z-[80] bg-slate-900/40" @click.self="showAddCustomerModal = false">
        <div x-show="showAddCustomerModal" x-transition:enter="transition ease-out duration-250"
            x-transition:enter-start="translate-y-full" x-transition:enter-end="translate-y-0"
            x-transition:leave="transition ease-in duration-200"
            x-transition:leave-start="translate-y-0" x-transition:leave-end="translate-y-full"
            class="absolute bottom-0 left-0 right-0 bg-white rounded-t-[1.5rem] max-h-[90vh] overflow-y-auto">
            ...
            <div class="px-5 py-4 bg-primary sticky top-0 z-10">
                <h2 class="text-white font-bold text-[16px]">Add New Customer</h2>
            </div>

            <form @submit.prevent="submitNewCustomer()" class="p-5 flex flex-col gap-4">
                <div>
                    <label class="text-[11px] font-bold text-gray-700 uppercase tracking-wider mb-1.5 block">Customer Name <span class="text-primary">*</span></label>
                    <input type="text" x-model="newCustomerForm.name" required placeholder="Enter full name"
                        class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-[14px] font-medium text-gray-700 outline-none">
                </div>

                <div>
                    <label class="text-[11px] font-bold text-gray-700 uppercase tracking-wider mb-1.5 block">Phone No. <span class="text-primary">*</span></label>
                    <input type="text" x-model="newCustomerForm.phone" required inputmode="numeric" oninput="this.value = this.value.replace(/[^0-9]/g, '')" placeholder="Enter phone number"
                        class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-[14px] font-medium text-gray-700 outline-none">
                </div>

                <div>
                    <label class="text-[11px] font-bold text-gray-700 uppercase tracking-wider mb-1.5 block">Opening Balance</label>
                    <input type="number" min="0" step="0.01" x-model="newCustomerForm.amount_balance" placeholder="0.00"
                        class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-[14px] font-medium text-gray-700 outline-none">
                </div>
                ...
                <div class="flex gap-3 mt-2">
                    ...
                    <button type="submit" :disabled="savingCustomer"
                        class="flex-1 py-3.5 bg-primary text-white font-bold rounded-xl text-[13px] uppercase tracking-wide flex items-center justify-center gap-2"
                        :class="savingCustomer ? 'opacity-60' : ''">
                        <i class="bi" :class="savingCustomer ? 'bi-arrow-repeat animate-spin' : 'bi-check2-circle'"></i>
