...
Route::get('/dashboard', function () {
    ...
    // Account Receivable = sum of what customers still owe us.
    // Uses Customer.amount_balance directly (opening_balance + sales - payments received).
    // Only positive balances are summed — negative means a credit owed TO the customer,
    // which is a liability not a receivable.
    $accountsReceivable = \App\Models\Customer::where('amount_balance', '>', 0)
        ->sum('amount_balance');
...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    Route::middleware('permission:Sales & POS')->group(function () {
        ...
        Route::get('/payment-in/print/{id}', [App\Http\Controllers\PaymentInController::class, 'printReceipt'])->name('payment_in.print')->middleware('tenant.owns:payment_ins');
        ...
        Route::get('/sales-return/{id}/pdf', [App\Http\Controllers\SalesReturnController::class, 'downloadPdf'])->name('sales.return.pdf');
